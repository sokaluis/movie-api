...
const getMovies = async (req, res) => {
  try {
    ...
    if (req.query.genre || req.query.title) {
      const dataFiltered = JSON.parse(data).filter(
        (movie) =>
          (req.query.genre !== undefined && movie.genre === req.query.genre) ||
          (req.query.title !== undefined && queryTitle.test(movie.title))
      );
      return res.json(dataFiltered);
    }
    ...
    if (req.query.limit) {
      ...
      if (limit < min || limit > max)
        return res.status(400).json({
          status: 400,
          message: "Limit must not to be minor than 1 or greater than 20",
        });
      const dataFiltered = JSON.parse(data).filter(
        (movie, index) => index < limit
      );
      return res.json(dataFiltered);
    }
